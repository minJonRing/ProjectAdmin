    ...
    user.findOne({ username, password }).exec((err, doc) => {
      try {
        if (!err && doc) {
          ...
          setRedis(token)
          r({ status: 200, msg: '数据请求成功', data: { token } })
        } else {
          r({ status: 203, msg: '账号或密码不正确', data: null })
        }
      } catch (e) {
...
router.get('/userinfo', async (ctx, next) => {
  ...
  try {
    ...
    ctx.body = { status: 201, msg: '获取用户信息出错', data: error }
  }
  console.log(id)
  const data = await new Promise((r, reject) => {
    user.findOne({ id }).exec((err, doc) => {
      try {
        if (!err && doc) {
          const {
            avatar,
          ...
          r({ status: 203, msg: '获取用户信息出错', data: null })
        }
      } catch (e) {
        r({ status: 203, msg: e, data: '' })
      }
